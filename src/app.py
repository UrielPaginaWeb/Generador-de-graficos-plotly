...
def generar_grafico_proceso(tipo_grafico, params, process_id, queue):
    try:
        ...
        logger.info(
            f"[PROCESO HIJO] PID: {hijo_pid} - Iniciando generación de {tipo_grafico}"
        )  # informacion cuando inicia
        if tipo_grafico == "boxplot":
            generador_graficos.generar_grafico_boxplot(
                **params # ** sirve para recoger múltiples argumentos con nombre automáticamente.
            )  # genera el grafico boxplot
        elif tipo_grafico == "barras":
            generador_graficos.generar_grafico_barras(
                **params
            )  # genera el grafico barras
        ...
        logger.info(
            f"[PROCESO HIJO] PID: {hijo_pid} - {tipo_grafico} generado con éxito"
        )
...
def generador_boxplot():
    ...
    process_id = str(
        uuid.uuid4()
    )  # genera ID único para cada solicitud para crear graficos
    ...
    while True:
        try:
            ...
            if not result_queue.empty():
                ...
                )  # se recupera un elemento de la cola
                if (
                    current_task_id == process_id
                ):  # se verifica que el elemento coincida con su id
                    p.join()  # Esperar a que termine el proceso
                    return jsonify(result)

            # Verificar timeout
            if (
                time.time() - timeStart > timeOut
            ):  # Verifica si se pasa del tiempo de 30s
...
def generador_barras():
    ...
    process_id = str(
        uuid.uuid4()   # genera ID único para cada solicitud para crear graficos
    ) 
...
def generador_mapa():
    ...
    # Crear y iniciar clase Process para iniciar el proceso
    p = mp.Process(
        target=generar_grafico_proceso, args=("map", params, process_id, result_queue)
    )
    p.start()
